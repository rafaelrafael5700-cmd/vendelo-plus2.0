        ...
        const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxNnGeSAqW59ULVJmmGNj3mPjy6O95U5DSXMVkGZ01cRo4TFG_x9LWEbt1Lf8DLMZMuwQ/exec";
        const MASTER_URL = "https://script.google.com/macros/s/AKfycbxG8L3F26OkAIBnj9Abz6VD5wfV5t8Try6swqpam1A5z8MwVdi3ZgE86IX58eBBghOQVw/exec";
        ...
        async function api(payload) {
            // Inyectar el ID de la base de datos específica del usuario
            if (usuarioActual && usuarioActual.sheetId) {
                payload.sheetId = usuarioActual.sheetId;
            }

            // Permitir acciones de acceso aunque la licencia no esté activa aún
            const bypassLicense = ["loginUsuario", "registrarUsuario"].includes(payload.accion);
            if (!window.licenciaActiva && !bypassLicense) {
                return { ok: false, error: "Licencia suspendida" };
            }
    ...
    async function cargarDatosIniciales() {
        ...
        try {
            ...
            if (resp && resp.ok) {
                ...
                throw new Error("Error backend al cargarTodo");
            }
        } catch (err) {
            const cache = localStorage.getItem('vendelo_cache');
            if (cache) {
                aplicarDatos(JSON.parse(cache));
                console.warn("MODO LOCAL (Sin conexión)");
            } else {
                mostrarToast("Error crítico: No hay conexión ni datos en caché local.");
            }
        }
    }
        ...
        async function revalidarLicencia() {
            try {
                ...
                if (data.permitido === false) {
                    window.licenciaActiva = false;
                    if(badge) {
                        badge.innerText = "SUSPENDIDO: " + (data.plan || "");
                        badge.className = "text-[10px] font-black uppercase text-red-600 tracking-widest";
                        document.getElementById('status-dot').classList.replace('bg-green-500', 'bg-red-500');
                    }
                    mostrarToast("⚠️ Acceso Suspendido. Por favor, renueva tu licencia.");
                    // Podríamos crear un overlay modal aquí como en el móvil.
                } else {
                    window.licenciaActiva = true;
                    if(badge) {
    ...
    function renderSalesHistory(busqueda = "") {
        ...
        if(busqueda.trim() !== "") {
            ...
            filtrados = filtrados.filter(v => {
                const cliente = (v.cliente || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    ...
    function renderClients(busqueda = "") {
        ...
        tbody.innerHTML = filtrados.map(c => `
            <tr class="hover:bg-slate-50/50 transition-all">
                <td class="px-8 py-5 text-sm font-bold text-slate-800 uppercase">${c.nombre}</td>
                <td class="px-8 py-5 text-xs font-bold text-slate-500">${c.telefono || 'N/A'}</td>
                <td class="px-8 py-5 text-xs font-black text-red-600">$${parseFloat(c.deuda || 0).toFixed(2)}</td>
                <td class="px-8 py-5 text-right">
                    <button onclick="verDetalleClienteDesktop('${c.id}')" class="text-blue-600 hover:text-blue-800"><i class="ph-bold ph-eye text-lg"></i></button>
                </td>
            </tr>
        `).join('');
    }
    ...
    function verDetalleClienteDesktop(id) {
        ...
        if(!c) return;
        let html = `
            <h3 class="text-2xl font-black text-slate-800 uppercase mb-4">${c.nombre}</h3>
            <p class="text-xs font-bold text-slate-400 mb-8">Deuda Actual: $${parseFloat(c.deuda||0).toFixed(2)}</p>
            <button onclick="toggleModalDesktop(false)" class="w-full bg-slate-100 py-4 rounded-2xl font-black uppercase text-xs">Cerrar</button>
        `;
        document.getElementById('modal-inner-html').innerHTML = html;
        toggleModalDesktop(true);
    }
    ...
    function buscarProductoAjuste(q) {
        ...
        const query = q.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
        const filtrados = productos.filter(p => p.nombre.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").includes(query)).slice(0, 5);
    ...
    function logout() {
        if(confirm("¿Estás seguro que deseas cerrar sesión?")) {
            localStorage.removeItem("usuarioVentasPlus");
            location.reload();
        }
    }
    ...
    async function iniciarSesionDesktop() {
        ...
        if(!u || !p) {
            Swal.fire({ icon: 'warning', title: 'Datos incompletos', text: 'Ingresa usuario y contraseña' });
            return;
        }
        ...
        try {
            // EL LOGIN AHORA SE HACE AL MAESTRO PARA OBTENER EL sheetId
            ...
            if (res && res.ok) {
                ...
                Swal.fire({ icon: 'error', title: 'Acceso Denegado', text: res.error || "Credenciales incorrectas" });
            }
        } catch (e) {
            Swal.fire({ icon: 'error', title: 'Error de conexión', text: 'No se pudo conectar con el servidor maestro' });
        }
    }
    ...
    async function registrarUsuarioDesktop() {
        ...
        try {
            const correoValido = u.includes('@') ? u : u + "@vendelo.plus";
            const idLimpio = u.split('@')[0];

            // EL REGISTRO AHORA CREA LA DB EN EL MAESTRO
            ...
            if (res.ok) {
                ...
                Swal.fire({ icon: 'error', title: 'Error', text: res.error || "Error al registrar" });
            }
        } catch (e) {
            Swal.fire({ icon: 'error', title: 'Error de conexión', text: 'No se pudo conectar con el servidor maestro' });
        }
